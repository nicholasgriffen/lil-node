...
module.exports = {
    run: function(assert) {
        ...
        console.log('authorization#login takes a req and res, sets res.statusCode to 201, sets res.Set-Cookie to jwt=jwt and calls res.end with a JSON string containing the message Logged in and a data object with a jwt key matching the jwt cookie')
        auth.login(req, res)
        ...
        assert.equal(res['Set-Cookie'], `jwt=${res.data.data.jwt}`) 
